...
public class WoolInterpolations {
    ...
    public static final Interpolation CATMULL_ROM = register(
            "catmull_rom", (AdvancedInterpolation) (t, a, b, previous, next) ->
                    0.5F * (2 * a + (b - previous) * t + (2 * previous - 5 * a + 4 * b - next) * t * t + (next - previous + 3 * (a - b)) * t * t * t)
    );


    private static Interpolation register(String name, Interpolation interpolation) {
        return Registry.register(WoolRegistries.INTERPOLATION, Identifier.of(Wool.MOD_ID, name), interpolation);
    }

    public static void initialize() {
        Wool.logInitializing("interpolations");
    }
}
