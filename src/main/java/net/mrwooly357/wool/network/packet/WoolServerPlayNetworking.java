...
public class WoolServerPlayNetworking {
    ...
    public static void initialize() {
        Wool.logInitializing("server play networking");
        ...
        ServerPlayNetworking.registerGlobalReceiver(OpenPlayerAccessoryInventoryC2SPacket.ID, (payload, context) ->
                context.player().openHandledScreen(
                        new SimpleNamedScreenHandlerFactory((syncId, playerInventory, player) ->
                                new PlayerAccessoryInventoryScreenHandler(syncId, player), Text.translatable("gui." + Wool.MOD_ID + ".player_accessory_inventory")))
        );
        PayloadTypeRegistry.playC2S().register(CanTickAnimationUpdateC2SPacket.ID, CanTickAnimationUpdateC2SPacket.CODEC);
        ServerPlayNetworking.registerGlobalReceiver(CanTickAnimationUpdateC2SPacket.ID, (payload, context) ->
                ((Animatable.Server) context.player().getWorld().getEntityById(payload.entityId())).setCanTickAnimation(payload.canTickAnimation())
        );
        PayloadTypeRegistry.playC2S().register(ElapsedAnimationTicksSyncC2SPacket.ID, ElapsedAnimationTicksSyncC2SPacket.CODEC);
        ServerPlayNetworking.registerGlobalReceiver(ElapsedAnimationTicksSyncC2SPacket.ID, (payload, context) ->
                ((Animatable.Server) context.player().getWorld().getEntityById(payload.entityId())).setElapsedAnimationTicks(payload.elapsedAnimationTicks())
        );
    }
}
